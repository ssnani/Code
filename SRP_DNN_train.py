...
import sys 
import random
from dataset_v2 import MovingSourceDataset
from array_setup import get_array_set_up_from_config

from SRP_DNN_network import SRPDNN
from SRP_DNN_Input import SRPDNN_features
from loss_criterion import MSELoss_vad
...
class SRPDNN_Model(pl.LightningModule):
	def __init__(self, num_freq, bidirectional, train_dataset, val_dataset, batch_size, num_workers):
		...
		self.model = SRPDNN(num_freq, bidirectional) 
		self.loss = MSELoss_vad()
	...
	def configure_optimizers(self):
		_optimizer = torch.optim.Adam(self.model.parameters(), lr=1e-3) #weight_deacy=1e-5
		#_lr_scheduler = torch.optim.lr_scheduler.StepLR(_optimizer, step_size=10, gamma=0.98) #torch.optim.lr_scheduler.ReduceLROnPlateau(_optimizer, mode='min',factor=0.5, patience=3)
	...
	def forward_step(self, input_batch, batch_idx, mode_str):
		log_ms_ph, tgt_dp_phdiff, doa = input_batch
		pred_phdiff, tgt_dp_phdiff = self.forward(input_batch)
		loss = self.loss(pred_phdiff, tgt_dp_phdiff, doa)
		self.log(f'{mode_str}_loss', loss, on_step=True, on_epoch=True, prog_bar=True, logger=True)
		return {"loss" : loss , "est_phdiff" : pred_phdiff }


	def training_step(self, train_batch, batch_idx):
		return self.forward_step(train_batch, batch_idx, 'train')


	def validation_step(self, val_batch, batch_idx):
		return self.forward_step(val_batch, batch_idx, 'val')
...
def main(args):
	...
	train_dataset = MovingSourceDataset(dataset_file, array_config, transforms=[ SRPDNN_features(320, 160, array_config['array_type'], array_config['array_setup'])], 
	...
	dev_dataset = MovingSourceDataset(val_dataset_file, array_config, transforms=[ SRPDNN_features(320, 160, array_config['array_type'], array_config['array_setup'])],
									T60=T60, SNR=SNR, dataset_dtype=dataset_dtype, dataset_condition=dataset_condition, train_flag=args.train,
									noise_simulation=noise_simulation, diffuse_files_path=diffuse_files_path) #, size=20
	...
	model = SRPDNN_Model(160, bidirectional, train_dataset, dev_dataset, args.batch_size, args.num_workers)
	...
	trainer = pl.Trainer(accelerator='gpu', devices=args.num_gpu_per_node, num_nodes=args.num_nodes, precision=precision,
					...
					callbacks=[checkpoint_callback, model_summary, lr_monitor],
					logger=tb_logger,
