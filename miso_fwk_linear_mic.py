	...
	def get_model_paths_input(self, parse_info: list [list]):
		...
		for idx, lst_val in enumerate(parse_info):
			...
			for lst_val_ch in lst_val:
				net_inp, net_out, model_path, loss_wgt_mech = lst_val_ch[0], lst_val_ch[1], lst_val_ch[2], lst_val_ch[3]
			
				loss_flag = model_path.split('/')[-6] #4
...
if __name__=="__main__":
	...
	models_info = []
	ckpt_dirs = ['/fs/scratch/PAS0774/Shanmukh/ControlledExp/random_seg/Linear_array_8cm_dp_rir_t60_0' ]
	
	for ckpt_dir in ckpt_dirs:
		for _loss_flag in loss_list:
			models = []
			for ref_mic_idx in ref_mic_idx_list:
				...
				if os.path.exists(ckpt_dir_1):
					for _file in os.listdir(ckpt_dir_1):
						if _file.endswith(".ckpt") and _file[:5]=="epoch":
							models.append((net_inp, net_out, os.path.join(ckpt_dir_1, _file), loss_wgt_mech))
