...
class Multi_task_MIMO_Net(nn.Module):
    def __init__(self, bidirectional, net_inp, net_out, num_doa_classes):
        super(Multi_task_MIMO_Net, self).__init__()
        ...
        self.doa_linear_layer = nn.Linear(160*16, self.num_doa_classes)
    ...
    def forward(self, x):
        ...
        #breakpoint()
        out_doa = torch.concat((out1, out2), dim=2)

        doa_logits = self.doa_linear_layer(out_doa)
            
        return out, doa_logits


def test_model(bidirectional, num_inp, num_out, num_doa_classes):
    feat = torch.randn(10, num_inp, 100, 161)
    net = Multi_task_MIMO_Net(bidirectional, num_inp, num_out, num_doa_classes) #MIMO_Net(bidirectional, num_inp, num_out)
    ...
    import time
    start = time.time()
    est, doa_logits = net(feat)
    end = time.time() - start 
    print('Time for {} -> {}, {} is {} sec'.format(feat.shape, est.shape, doa_logits.shape, end))

if __name__=="__main__":
    #import sys
    #bidirectional = sys.argv[1] == "True"
    test_model(bidirectional=True, num_inp=4, num_out=2, num_doa_classes=181)